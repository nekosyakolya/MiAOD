...
void CBohr::Add(const std::wstring &s)
{
	...
	{
		wchar_t ch = std::tolower(s[i], std::locale::empty());
		if (m_bohr[num].nextItems.find(ch) == m_bohr[num].nextItems.end())
		{
			m_bohr.push_back(MakeBohrItem(num, ch));
			m_bohr[num].nextItems.emplace(ch, m_bohr.size() - 1);
		}
